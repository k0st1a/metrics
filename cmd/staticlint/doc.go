...
# Используемые анализаторы
  ...
  - собственный анализаторы:

    -- osexit, который запрещает прямой вызова os.Exit в функции main пакета main.

    -- собственный анализатор skipgenerated, который skip-ает файлы для статического анализа другими
    анализаторами, если файл начинается с комментария "// Code generated by " 
*/
package main
